...
# Sort by name for a structured dropdown
...
class Wallet:
    ...
    def sell(self, ticker, price, amount, date): # <--- ADDED 'date' parameter
        current = self.get_holding(ticker)
        if current['amount'] >= amount and amount > 0:
            ...
            # ... (removed for brevity, ensure the original logic is used) ...
...
start_d = st.date_input("Start", pd.to_datetime("2020-01-01"))
...
indicators_raw = st.text_area("Define Indicators", "sma_50, sma_200, rsi_14, bb_20, macd, stoch_14, ema_200")
...
if 'buy_rules' not in st.session_state:
    st.session_state.buy_rules = [
        {"type": "% Cash", "cond": "amount == 0  and macd_histogram > 0 and macd < 0  and close > ema_200", "size": 2.0},
        {"type": "% Position", "cond": "pct_profit < -0.05", "size": 200.0} 
    ]

# Function to add a new blank rule
def add_rule():
    st.session_state.buy_rules.append({"type": "% Cash", "cond": "", "size": 0.0})
...
for i, rule in enumerate(st.session_state.buy_rules):
    ...
    with c4:
        rule['size'] = st.number_input("Size %", value=rule['size'], key=f"rule_s_{i}", label_visibility="collapsed", step=1.0)
...
if 'sell_rules' not in st.session_state:
    st.session_state.sell_rules = [
        {"cond": "macd_histogram < 0 and macd > 0 and pct_profit > 0.1", "size": 100.0}
    ]

# Function to add a new blank sell rule
def add_sell_rule():
    st.session_state.sell_rules.append({"cond": "", "size": 0.0})
...
for i, rule in enumerate(st.session_state.sell_rules):
    ...
    with c3:
        rule['size'] = st.number_input("Size %", value=rule['size'], key=f"sell_rule_s_{i}", label_visibility="collapsed", step=1.0)
    ...
    if not tickers_input:
        ...
        if raw_df.empty:
            ...
            if wallet.transaction_history:
                ...
                # Create the DataFrame from the logged history
                history_df = pd.DataFrame(wallet.transaction_history)
                
                # Format the data for display
                history_df['Price'] = history_df['Price'].apply(lambda x: f"{x:.2f}")
                history_df['Total Value'] = history_df['Total Value'].apply(lambda x: f"{x:,.2f}")
                history_df['Date'] = history_df['Date'].dt.strftime('%Y-%m-%d')
                
                # Sort by Ticker, then Date
                history_df.sort_values(by=['Ticker', 'Date'], ascending=[True, True], inplace=True)
                
                # Group the history by Ticker for a cleaner, expanded view
                unique_tickers = history_df['Ticker'].unique()
                
                for ticker in unique_tickers:
                    ticker_df = history_df[history_df['Ticker'] == ticker].drop(columns=['Ticker'])
                    # Use an expander for each stock's history
                    with st.expander(f"🔍 **{ticker}** ({len(ticker_df)} Transactions)"):
